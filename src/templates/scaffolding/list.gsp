        ...
        <tbody>
            <g:each in="\${${propertyName}List}" status="i" var="${propertyName}">
                <tr>
                    ...
                    <td class="hidden-phone"><g:formatBoolean boolean="\${${propertyName}.${p.name}}"/></td>
                    <% } else if (p.type == Date || p.type == java.sql.Date || p.type == java.sql.Time || p.type == Calendar) { %>
                    <td class="hidden-phone"><g:formatDate date="\${${propertyName}.${p.name}}"/></td>
                    <% } else { %>
                    <td class="hidden-phone">\${fieldValue(bean: ${propertyName}, field: "${p.name}")}</td>
                    <% }
                    }
                    } %>
                </tr>
